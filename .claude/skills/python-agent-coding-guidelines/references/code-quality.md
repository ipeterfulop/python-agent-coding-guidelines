...
Default to plain `for` loops and explicit `try` / `finally` when an idiom would
reduce clarity.

Use comprehensions only when all are true:
...
Use generators only when data is large, unbounded, streamed, consumed one item at
a time, or expensive to materialize. Return a list or tuple when callers need
