...
description: "Use when working on Python repositories where Codex should apply uv-first, reviewable coding guidelines adapted from this repo's Claude Code instruction file: preserve behavior, make small diffs, avoid speculative abstractions, improve touched code, use clear names and type hints, and run pytest, ruff, and type-check checks through uv."
---

# Creating Skills From The Instruction File

Apply the Python coding-agent rules from this repository's `.claude/CLAUDE.md`
as a reusable workflow for Python code changes. The source instruction file is
optimized for Claude Code, and this skill adapts the same engineering guidance
for explicit Codex skill invocation.
